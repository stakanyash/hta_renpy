...
init -1500 python in _console:
    ...
    @command(_("addgold <amount>: add gold to player"))
    def addgold(l):
        amount_str = l.rest().strip()
        
        if not amount_str:
            return "Usage: addgold <amount>"
        
        try:
            amount = renpy.python.py_eval(amount_str)
            
            store.player_config.add_money(amount)
        except Exception as e:
            return "Error: {}".format(str(e))

    @command(_("car <number>: change player car (1=Van, 2=Molokovoz, 3=Ural)"))
    def car(l):
        val_str = l.rest().strip()
        ...
        try:
            ...
            if val not in car_map:
                return "Error: number must be 1-3"
            ...
            new_max_heal = store.CarMaxHeals.get(car_name, 15)
            ...
            if car_name == "Ural":
                ...
                store.player_config.big_gun_install = "NotPossible"
            
            return "Car changed to: {} (HP: {}, Heals: {})".format(
                car_name,
                new_max_hp,
                new_max_heal
            )
            ...
            return "Error: value must be a number 1-3"
        except Exception as e:
            return "Error: {}".format(str(e))

    @command(_("additem <item_id>: add item to inventory"))
    def additem(l):
        item_str = l.rest().strip()
        ...
        try:
            ...
            if store.player_config.try_add_item(item_str):
                ...
                max_inv_size = store.CarInventoryLimits.get(store.player_config.car, 4)
                return "Added: {} ({}). Inventory: {}/{}".format(
                    item_name, 
                    item_str,
                    current_inv_size,
                    max_inv_size
                ...
                return "Error: inventory is full! ({}/{})".format(
                    len(store.player_config.inventory),
                    store.CarInventoryLimits.get(store.player_config.car, 4)
                )
            
        except Exception as e:
            return "Error: {}".format(str(e))

    @command(_("giveguns: give random weapon based on current car"))
    def giveguns(l):
        try:
            import random
            ...
            if weapon_data:
                ...
                store.player_config.gun_type = None
            
            weapon_size = "Big" if weapon in store.bigweapon_prices else "Small"
            
            return "Given weapon: {}, {}".format(weapon, weapon_size)

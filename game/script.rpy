...
label felixbeforefight:
    ...
    menu:
        "Атаковать охрану":
            ...
            # Temporary "return" (replace with jump to titles screen)
            return
...
label felixbase:
    ...
    if TakeGunFromZaimka == "True":
        "Бой был посложнее, чем с его охраной, но вы смогли одержать победу и Феликс сдался."
        jump felixdefeated
    elif TakeGunFromZaimka == "False":
        "Но к сожалению ваш грузовик и так не был в лучшем состоянии, а тут он ещё изрядно пострадал при битве с охраной и Феликсу это сыграло на руку."
        "Он достаточно легко расправился с вашей машиной..."
        mc "{cps=5}Это... конец..."
        # Temporary "return" (replace with jump to title screen)
        return
